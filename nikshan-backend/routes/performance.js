...
router.get("/team", async (req, res) => {
  try {
    ...
    // Get all sales staff
    const { data: users } = await db.from("nk_users").select("id, name, role, store_id, nk_stores(name)").in("role", ["sales_exec","sales_manager","branch_manager"]).eq("active", true);

    // Get leads assigned and won in period
    const { data: leads } = await db.from("nk_leads").select("id, assigned_to, converted_by, status, estimated_value, won_at").gte("created_at", fromDate).lte("created_at", toDate);
    const { data: wonLeads } = await db.from("nk_leads").select("id, converted_by, estimated_value").eq("status", "won").gte("won_at", fromDate).lte("won_at", toDate);
    const { data: purchases } = await db.from("nk_purchases").select("id, sold_by, amount").gte("created_at", fromDate).lte("created_at", toDate);
    const { data: attendees } = await db.from("nk_customer_interactions").select("user_id, customer_id").gte("created_at", fromDate).lte("created_at", toDate);
