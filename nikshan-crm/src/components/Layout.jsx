...
import {
  LayoutDashboard, Users, TrendingUp, ShoppingBag, CreditCard,
  Wrench, Calendar, BarChart3, FileText, LogOut, Menu, X, ChevronRight
} from 'lucide-react'
...
const ROLE_COLORS = { admin:'bg-purple-100 text-purple-700', branch_manager:'bg-blue-100 text-blue-700', sales_manager:'bg-indigo-100 text-indigo-700', sales_exec:'bg-green-100 text-green-700', technician:'bg-orange-100 text-orange-700' }
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className={`${collapsed ? 'w-16' : 'w-60'} bg-white border-r border-slate-100 flex flex-col transition-all duration-200 shrink-0`}>
        ...
        <div className={`flex items-center gap-3 px-4 py-5 border-b border-slate-100 ${collapsed ? 'justify-center' : ''}`}>
          ...
          {!collapsed && <div><div className="font-bold text-slate-900 text-sm leading-tight">IPIX CRM</div><div className="text-xs text-slate-400">Nikshan Electronics</div></div>}
        </div>
          ...
          <div className="p-3 border-t border-slate-100">
            <div className="flex items-center gap-2 px-2 py-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${ROLE_COLORS[user.role] || 'bg-slate-100 text-slate-600'}`}>{ROLE_LABELS[user.role] || user.role}</span>
              </div>
            </div>
            <button onClick={handleLogout} className="mt-1 w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors">
        ...
        {collapsed && (
          <div className="p-2 border-t border-slate-100">
            <button onClick={handleLogout} className="w-full flex items-center justify-center p-2 text-red-500 hover:bg-red-50 rounded-lg" title="Logout"><LogOut size={16} /></button>
          </div>
        )}
      </aside>
